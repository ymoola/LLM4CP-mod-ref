...
def build_model(n_days_per_week, n_weeks, s_min, s_max, requirements):
    """
    Rotating Rostering feasibility model - CR2.

    CR2: Any sequence of night shifts must be followed immediately by a rest
    day before any other working shift can be assigned.
    i.e. if x[i] == NIGHT and x[i+1] != NIGHT, then x[i+1] must be OFF.
    ...
    # ------------------------------------------------
    # 8. CR2: Night shift run must be followed by a rest day
    # If today is night and tomorrow is not night,
    # then tomorrow must be a rest day
    # ------------------------------------------------
    for i in range(n_days):
        nxt = (i + 1) % n_days
        model += ((x[i] == NIGHT) & (x[nxt] != NIGHT)).implies(x[nxt] == OFF)
