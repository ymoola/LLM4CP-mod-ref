...
def build_model(n_days_per_week, n_weeks, s_min, s_max, requirements):
    """
    Rotating Rostering feasibility model - CR3.

    CR3: After a rest day, an employee may not be assigned directly to a
    night shift on the next day.
    i.e. if x[i] == OFF then x[(i+1) % n_days] != NIGHT.
    ...
    # ------------------------------------------------
    # 8. CR3: No night shift directly after a rest day
    # ------------------------------------------------
    for i in range(n_days):
        nxt = (i + 1) % n_days
        model += (x[i] == OFF).implies(x[nxt] != NIGHT)
