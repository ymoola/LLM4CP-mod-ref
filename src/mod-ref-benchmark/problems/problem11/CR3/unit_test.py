...
def cr3_verify_func(data_dict, hypothesis_solution):
    """
    Verify Rotating Rostering CR3:
    - Base constraints: link, weekend match, min/max run, rest days, forward order, requirements
    - CR3 constraint: a rest day may not be immediately followed by a night shift
    """
    ...
    # 2) Type and shape checks — x_roster
    ...
    # 2b) Type and shape checks — y_roster
    ...
    # 10) CR3: no night shift directly after a rest day
    for i in range(n_days):
        nxt = (i + 1) % n_days
        if x_roster[i] == OFF:
            assert x_roster[nxt] != NIGHT, (
                f"CR3 violated at day {i}->{nxt}: rest day followed directly "
                f"by a night shift"
            )
