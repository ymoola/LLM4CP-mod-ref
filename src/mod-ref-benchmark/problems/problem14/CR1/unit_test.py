...
def cr1_verify_func(data_dict, hypothesis_solution):
    ...
    balance_rules = data_dict["balance_rules"]
    ...
    for idx, rule in enumerate(balance_rules):
        ...
        diff = abs(section_a_load - section_b_load)
        assert diff <= rule["tolerance"], (
            f"Balance rule {idx} violated: section loads {section_a_load} and {section_b_load} "
            f"differ by {diff}, tolerance is {rule['tolerance']}."
        )
