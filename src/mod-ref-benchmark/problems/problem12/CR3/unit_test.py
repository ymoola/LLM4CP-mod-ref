...
def cr3_verify_func(data_dict, hypothesis_solution):
    """
    Verify Steel Mill Slab Design CR3:
    - Base constraints: load computation, loss computation, at-most-two-colors
    - CR3 objective terms: total split penalty and combined objective
    """
    ...
    try:
        ...
        total_split_penalty = hypothesis_solution["total_split_penalty"]
        total_loss = hypothesis_solution["total_loss"]
        total_objective = hypothesis_solution["total_objective"]
    except Exception as e:
    ...
    assert isinstance(total_split_penalty, int), "total_split_penalty must be an int"
    assert total_split_penalty >= 0, "total_split_penalty must be non-negative"

    assert isinstance(total_loss, int), "total_loss must be an int"
    assert total_loss >= 0, "total_loss must be non-negative"

    assert isinstance(total_objective, int), "total_objective must be an int"
    assert total_objective >= 0, "total_objective must be non-negative"

    # 2) Verify slab loads match assignments
    ...
    # 3) Verify slab losses match loads
    for j in range(n_slabs):
        load = slab_loads[j]
        if load == 0:
            expected_loss = 0
    ...
    # 4) At most two distinct colors per slab
    for j in range(n_slabs):
        colors_on_slab = set(
    ...
    # 5) CR3: recompute colour split penalty
    color_slab_count = {color: 0 for color in all_colors}
    for color in all_colors:
        for j in range(n_slabs):
            if any(
                order_to_slab[i] == j and colors[i] == color
                for i in range(n_orders)
            ):
                color_slab_count[color] += 1

    recomputed_split_penalty = sum(
        max(0, color_slab_count[color] - 1) for color in all_colors
    )
    assert total_split_penalty == recomputed_split_penalty, (
        f"Split penalty mismatch: reported {total_split_penalty}, "
        f"recomputed {recomputed_split_penalty}"
    )
    ...
    recomputed_total_objective = recomputed_total_loss + recomputed_split_penalty
    assert total_objective == recomputed_total_objective, (
        f"total_objective mismatch: reported {total_objective}, "
        f"recomputed {recomputed_total_objective}"
    )
    ...
    if ref_opt_val is not None:
        ...
        sol_opt = "optimal" if recomputed_total_objective == ref_opt_val else "sat"
